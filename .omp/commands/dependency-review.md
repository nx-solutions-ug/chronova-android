...
Focus on `app/build.gradle`, `build.gradle`, `gradle/libs.versions.toml`, `Dockerfile`, and GitHub Actions workflow files.

## Step 2: Research Release Notes

For EACH updated dependency, find the actual changelog or release notes:
- **Android/Gradle libraries**: Check GitHub releases via `gh api /repos/{owner}/{repo}/releases` or inspect `CHANGELOG.md`.
