...
git diff "$BASE"...HEAD -- app/src/main/java/
git diff "$BASE"...HEAD -- app/src/main/res/
git diff "$BASE"...HEAD -- app/build.gradle
```
...
- **Lifecycle Safety**: All coroutines in Fragments/Activities MUST use `lifecycleScope`. UI updates MUST be guarded with `isAdded && _binding != null` before accessing views. Jobs started in `onViewCreated` MUST be cancelled in `onDestroyView()` (e.g. `loadDataJob?.cancel()`).
- **Repository Pattern**: All async data access MUST go through `ChronovaRepository`. Repository methods MUST return `Result<T>` — never throw from a repository method; always wrap in `try { Result.success(...) } catch (e: Exception) { Result.failure(e) }`. Direct API calls from UI (Fragments/Activities) bypassing the repository are a P1 violation.
- **Coroutines**: Use `lifecycleScope.launch { }` in Fragments/Activities. Handle `Result<T>` via `.onSuccess { }` / `.onFailure { }`. Never call `suspend` functions outside a coroutine scope.
- **Kotlin Naming Conventions**: Classes — PascalCase (`LanguagesPagerFragment`). Layouts — snake_case (`fragment_languages_pager.xml`). SharedPreferences keys — snake_case (`api_key`, `server_url`). Packages — lowercase dot-separated (`com.chronova.app.data`).
- **Fragment Factory Pattern**: Fragments with arguments MUST use `companion object { fun newInstance(...) = MyFragment().apply { arguments = Bundle().apply { ... } } }`. Never pass data via Fragment constructors.
- **Security**: No hardcoded credentials beyond what is documented in AGENTS.md. No cleartext secrets in logs. Validate all user-supplied input (server URL, API key) before use.
- **Code Quality**:
  - No `TODO`/`FIXME` left in production-path code without a corresponding issue.
  - No empty catch blocks — at minimum log the exception.
  - No dead code, unused variables, or unreachable branches.
  - No raw `Thread` or `Handler` usage — use coroutines.
...
gh pr-review review --add-comment $ARGUMENTS \
  -R $REPO_SLUG \
  --review-id "$REVIEW_ID" \
  --path "app/src/main/java/com/chronova/app/ui/main/MainStatsFragment.kt" \
  --line 42 \
  --side RIGHT \
...
- The suggestion block content MUST be valid replacement Kotlin/XML code without diff markers (`+`/`-`).
