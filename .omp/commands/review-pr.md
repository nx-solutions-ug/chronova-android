...
2. Review for: bugs, null safety (proper null handling, no unnecessary `!!` assertions), security, missing tests, hardcoded values.
3. Deduplicate against existing unresolved review threads.
4. Post review via `gh pr review $ARGUMENTS` — `REQUEST_CHANGES` for bugs/security/null safety, `APPROVE` for minor nits only.

## Step 5: Common checks (all review types)

- **Null safety**: No unnecessary non-null assertions (`!!`), proper nullable handling.
- **Error handling**: No empty catch blocks, no swallowed errors.
- **Security**: No exposed secrets, proper input validation, no hardcoded API keys.
