...
This is a Gradle/Kotlin Android project — dependencies are resolved by the Gradle wrapper, no `npm`/`package.json` exists. Verify the wrapper is executable:
...
- Search for similar bug fixes or feature implementations in `app/src/main/java/com/chronova/app/`.
- Read `AGENTS.md` for project conventions.
- Examine existing tests under `app/src/test/` and `app/src/androidTest/` for patterns to follow (note: these directories may be empty).
...
- Use **ViewBinding** — never `findViewById`. Always clear `_binding` in `onDestroyView()`.
- Use `lifecycleScope` for coroutines in Fragments/Activities.
- Return `Result<T>` from Repository methods.
- Access the repository via `ChronovaRepository(requireContext())` — no DI framework.
- Use Retrofit + Gson for networking (`ChronovaApiService`, `ApiClient`).
- Use SharedPreferences for auth/config persistence (`api_key`, `server_url`).
- Packages: `com.chronova.app.data` (data), `com.chronova.app.ui` (UI), `com.chronova.app.ui.main.cards` (cards).
- Classes: PascalCase — `LanguagesPagerFragment`, `LanguageAdapter`.
- Layouts: snake_case — `fragment_languages_pager.xml`, `item_language.xml`.
- Guard UI updates with `isAdded && _binding != null`.
- Cancel jobs in `onDestroyView()` (e.g., `loadDataJob?.cancel()`).
...
Place unit tests in `app/src/test/java/com/chronova/app/` and instrumented tests in `app/src/androidTest/java/com/chronova/app/`. Use JUnit 4 (`junit:junit:4.13.2`). Follow existing test patterns if any exist; otherwise create the directory structure and a basic test class. Use `tempfile`-style isolation where relevant.
...
- <Test 1>
- <Test 2>
- All quality gates pass (testDebugUnitTest, lintDebug, assembleDebug)"
```
...
- **MUST NOT** start a dev server, emulator, or attempt to launch the app. The CI environment has no Android emulator or display.
- **MUST NOT** open a browser or attempt visual verification. There is no display or user session available.
- **MUST NOT** attempt to connect to external services or databases. The CI environment is isolated.
- **MUST NOT** use `npm`, `node`, `pnpm`, or any JavaScript/TypeScript tooling — this is a Gradle/Kotlin project.
- When using `find` or `search` tools, ALWAYS pass `paths` as an array, not a string. Example: `find(paths=["app/src/**"])`, NOT `find(paths="app/src/**")`.
